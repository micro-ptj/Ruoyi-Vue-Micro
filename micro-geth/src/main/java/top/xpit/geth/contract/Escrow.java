...
public class Escrow extends Contract {
    ...
    public RemoteFunctionCall<TransactionReceipt> bid(BigInteger amount) {
        final org.web3j.abi.datatypes.Function function = new org.web3j.abi.datatypes.Function(
                FUNC_BID, 
                Arrays.<Type>asList(), 
                Collections.<TypeReference<?>>emptyList());
    ...
    public static RemoteCall<Escrow> deploy(Web3j web3j, Credentials credentials, ContractGasProvider contractGasProvider, BigInteger _startTime, BigInteger _biddingTime, String _beneficiary, BigInteger _uniqueItemId) {
        String encodedConstructor = FunctionEncoder.encodeConstructor(Arrays.<Type>asList(new Uint256(_startTime),
                new Uint256(_biddingTime),
                new Address(160, _beneficiary),
                new Uint256(_uniqueItemId)));
